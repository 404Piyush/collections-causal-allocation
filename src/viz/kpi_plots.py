...
def plot_kpi_summary(comparison_df: pd.DataFrame, output_path):
    ...
    _bar(axes[0, 0], comparison_df["actual_recovery_total_usd"],
         "Actual Recovery Total", "USD")
    _bar(axes[0, 1], comparison_df["cost_total_usd"],
         "Operational Cost", "USD")
    _bar(axes[1, 0], comparison_df["net_recovery_usd"],
         "Net Recovery", "USD")
    _bar(axes[1, 1], comparison_df["cost_per_dollar_recovered"],
         "Cost per $1 Recovered", "USD/$", "${:.3f}")
...
def plot_waterfall_improvement(comparison_df: pd.DataFrame, output_path):
    if not any(comparison_df["scenario"].str.contains("All Level 1")):
        return
    baseline = float(comparison_df.loc[
        comparison_df["scenario"].str.contains("All Level 1"), "net_recovery_usd"
    ].iloc[0])
    milp = float(comparison_df.loc[
        comparison_df["scenario"].str.contains("MILP"), "net_recovery_usd"
    ].iloc[0])
    delta = milp - baseline
    ...
    for bar, v in zip(bars, y):
        ax.text(bar.get_x() + bar.get_width() / 2, v + abs(baseline) * 0.01,
                f"${v:,.0f}", ha="center", fontsize=11)
    ax.annotate(
        "",
        xy=(1, milp), xytext=(0, baseline),
    ...
    ax.text(
        ...
        bbox=dict(boxstyle="round,pad=0.4", facecolor="#fef6b8", edgecolor="black"),
    )
