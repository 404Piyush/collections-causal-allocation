...
def plot_allocation_heatmap(assignments: pd.DataFrame, output_path):
    pivot = assignments.groupby(["channel", "agent_tier"]).size().unstack(fill_value=0)
...
def plot_pareto(sensitivity_df: pd.DataFrame, output_path):
    ...
    for cs in sorted(sensitivity_df["capacity_scale"].unique()):
        ...
        ax.plot(
            sub["budget"], sub["net_recovery"], "o-", lw=2, ms=8, label=f"Capacity scale = {cs:.2f}"
        )
...
def plot_tier_breakdown(assignments: pd.DataFrame, output_path):
    by_tier = (
        assignments.groupby("agent_tier")
        .agg(
            n=("account_id", "count"),
    ...
    w = 0.35
    ax.bar(x - w / 2, by_tier["cost"], width=w, color="#4575b4", label="Operational Cost ($)")
    ax.bar(x + w / 2, by_tier["net"], width=w, color="#1a9850", label="Net Recovery ($)")
    ax.set_xticks(x)
    ...
    for i, (c, n) in enumerate(zip(by_tier["cost"], by_tier["net"], strict=True)):
        ax.text(i - w / 2, c + max(by_tier["cost"]) * 0.01, f"${c:,.0f}", ha="center", fontsize=9)
        ax.text(i + w / 2, n + max(by_tier["cost"]) * 0.01, f"${n:,.0f}", ha="center", fontsize=9)
    fig.tight_layout()
    fig.savefig(output_path, dpi=140)
    plt.close(fig)
